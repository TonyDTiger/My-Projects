...
## Overview
This project analyzes a linear time invariant single mass spring damper system with an open-loop input force (either sinusoidal or unit step) and continuously applied on the single mass. Using linear algebra and linear controls theory, we can assess the long term behavior, responsiveness, and stability of this system. There are different mathematical definitions for what stability means, but overall we're looking to check if the system's states are bounded and does not grow infinitely. The system response (position state of the single mass) is evaluated in the time domain and in the frequency domain. As a fun experiment, a GUI was created to evaluate the change in the system's response relative to variations in the system's parameters and input force's frequency.
...
- Input: $\ u(t) = F(t) \$
- Output: Typically $\ y(t) = x(t) = x_1 \$, not an important piece for this analysis as we'll assume we have a perfect understanding of the state variables
...
Mentioned above, the purpose for all of this is to understand the system's response (amplification factor and phase shift) for an open-loop input force (either sinusoidal or unit step). If the system is a linear time invariant system, we can quickly assess the stability of the system through an evaluation of the state transition matrix's eigenvalues and checking if the real components are negative. From a different perspective, we can identify a couple key characteristics of the system's response to an input by running a sweep of sinusoidal input forces at different frequencies: 
  1. The frequency for when the system response starts to decay in amplification factor. This tells us the system's open-loop bandwidth or how responsive our system is. 
  2. If any, the frequency for when the system response has reversed polarity relative to the input force (ie. phase shift is 180 deg) and the associated amplification factor. This tells us how far the system is from instability. That is if we try to control this system at this critical frequency, this is equivalent to having the controls suddenly reversed which will inevitably result in a unstable system, similar to steering a car out of control when you're trying to steer left but the car turns right instead.
